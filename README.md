...
`ha_summary`, `primary_state`, and `secondary_state` also best-effort identify whether the queried IP is itself the unit being reported on, via `_determine_unit_role()` (same helper `ha_pair` uses, below; omitted if the platform doesn't populate the underlying OID). `ha_summary` appends a trailing note, e.g. `[10.56.1.226 = primary unit, currently active]`. For `primary_state`/`secondary_state`, the numeric state (and a cross-check note confirming it against the text role, since both OIDs report the same active/standby fact) is shown once rather than duplicated; if the queried IP *is* the unit being reported on, a trailing `[confirmed: <ip> is the <role> unit]` is appended, e.g. `Primary unit: Active (9) (confirmed by role text) [confirmed: 10.56.1.226 is the primary unit]`. If it *isn't* (e.g. querying the primary unit's IP with `--mode secondary_state`), a leading `NOTE: <ip> is the <role> unit; showing the <primary/secondary> unit's state instead - ` is prepended instead, so the caveat is read before the state data rather than after it.
